...
- Chain gRPC unary interceptors like **`internal/authn/app/service.go`** / **`internal/profile/app/service.go`**: `TraceUnaryInterceptor` (log `trace_id` / `x-trace-id`) → **`UnaryTracingInterceptor`** ([`pkg/shared/tracing`](pkg/shared/tracing) + [`infra/otel`](infra/otel); noop when tracer nil) → `UnaryProtovalidateInterceptor` → service **`RequestContextInterceptor`** → `RecoveryInterceptor` → `LoggerInterceptor` → `TimeoutInterceptor`. Log `trace_id` and OTel W3C trace ids may differ; OTel spans set attribute `muid.log_trace_id` when both are present.
