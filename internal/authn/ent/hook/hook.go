...
// Mutate calls f(ctx, m).
func (f UserPasskeyFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserPasskeyMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserPasskeyMutation", m)
}

// The UserRefFunc type is an adapter to allow the use of ordinary
// function as UserRef mutator.
type UserRefFunc func(context.Context, *ent.UserRefMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserRefFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserRefMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserRefMutation", m)
}
