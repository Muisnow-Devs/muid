...
NATS 事件 payload 使用 `api/proto/event/v1/profile.proto` 的 `ProfileChangedEvent`，其中 **`changed_fields`** 為 `google.protobuf.FieldMask`（路徑為 **`GetProfileResponse`** 頂層 proto 欄位名，例如 `display_name`、`email`）；**`changes`** 為 **`IdentityInformation`**（與 OIDC／部分更新共用欄位形狀）。**`occurred_at`** 為 **`google.protobuf.Timestamp`**。
