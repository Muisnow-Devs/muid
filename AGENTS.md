...
- For unexpected failures at RPC boundaries: log with **`pkg/log.LogUnexpected`** using **`log/slog.Attr`** (and **`log.WithAttrs`** for repeated safe fields); return **`grpcutils.GRPCInternalError()`** (fixed **`internal error`**) to clients. Use **`pkg/log`** for all logging (not stdlib **`log`**).
