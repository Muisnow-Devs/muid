...
func TestGetProfileResponsePathsFromUpdateRequestPaths(t *testing.T) {
	t.Parallel()
	t.Run("mixed identity paths", func(t *testing.T) {
		t.Parallel()
		got, err := GetProfileResponsePathsFromUpdateRequestPaths([]string{
			"identity.locale",
			"identity.name",
			"identity.email",
			"identity.picture",
		...
		want := []string{"avatar_url", "display_name", "email", "locale", "username"}
		if len(got) != len(want) {
			t.Fatalf("got %v want %v", got, want)
		}
		for i := range want {
	...
	t.Run("identity name maps to display_name", func(t *testing.T) {
		t.Parallel()
		got, err := GetProfileResponsePathsFromUpdateRequestPaths([]string{"identity.name"})
		if err != nil {
			t.Fatal(err)
		}
		if len(got) != 1 || got[0] != "display_name" {
			t.Fatalf("got %v", got)
		}
	})
	_, err := GetProfileResponsePathsFromUpdateRequestPaths(nil)
	if !errors.Is(err, ErrEmptyMask) {
		t.Fatalf("got %v", err)
	}
}
