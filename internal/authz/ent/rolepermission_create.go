...
func (_c *RolePermissionCreate) SetPermission(v string) *RolePermissionCreate {
...
func (_c *RolePermissionCreate) SetRole(v *OrganizationRole) *RolePermissionCreate {
...
func (_c *RolePermissionCreate) check() error {
	...
	if v, ok := _c.mutation.Permission(); ok {
		if err := rolepermission.PermissionValidator(v); err != nil {
			return &ValidationError{Name: "permission", err: fmt.Errorf(`ent: validator failed for field "RolePermission.permission": %w`, err)}
		}
	}
	if _, ok := _c.mutation.CreatedAt(); !ok {
